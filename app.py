...
def create_streamlit_app():
    ...
    # Run simulation
    eta, Z = run_cv_simulation(C, D, etai, etaf, v, n, alpha, k0, kc, T)
    ...
    with col2:
        ...
        st.metric("Simulation type", "EC Mechanism")
    ...
    max_current_idx = np.argmax(Z)
    min_current_idx = np.argmin(Z)
    
    ax.plot(eta[max_current_idx], Z[max_current_idx], 'ro')
    ax.text(eta[max_current_idx], Z[max_current_idx]*1.05, 
            f'({eta[max_current_idx]:.2f}V, {Z[max_current_idx]:.2f}mA/cm²)', 
            horizontalalignment='center')
    
    ax.plot(eta[min_current_idx], Z[min_current_idx], 'ro')
    ax.text(eta[min_current_idx], Z[min_current_idx]*1.1, 
            f'({eta[min_current_idx]:.2f}V, {Z[min_current_idx]:.2f}mA/cm²)', 
            horizontalalignment='center')
    
    st.pyplot(fig)
    
    # Download options
    csv_data = np.column_stack((eta, Z))
    st.download_button(
        label="Download Data as CSV",
        data=convert_to_csv(csv_data),
...
def convert_to_csv(data):
    import io
    csv_io = io.StringIO()
    np.savetxt(csv_io, data, delimiter=',', header='Potential (V),Current (mA/cm²)', comments='')
